...
app.controller('myCtrl', function($scope, $http) {
	...
	$scope.generateKeys(userName)
	{
		...
		downloadKey(privKey, "privateKey.txt");
	}

	// Let users download a .txt file containing their private key
	$scope.downloadKey(key, filename)
	{
	...
	$scope.encrypt()
	{
		...
		var uploaderPrivateKey = openpgp.key.readArmored(privKey).keys[0];
		...
		var options = 
		{
			...
			privateKeys: [uploaderPrivateKey]
		};

		openpgp.encrypt(options).then(encryptedData =>
		{
			// do something with encryptedData.data
		});

		// Get the unencrypted input text from the webpage
		// var original = document.getElementById("plaintext").value;
	...
	$scope.decrypt()
	{
		...
		var recipientPrivateKey = openpgp.key.readArmored(privKey).keys[0];
		var uploaderPublicKey = ;

		var options = 
		{
			message: openpgp.message.readArmored(encryptedData),
			publicKeys: openpgp.key.readArmored(uploaderPublicKey).keys,
			privateKeys: recipientPrivateKey
		};

		openpgp.decrypt(options).then(decryptedMessage =>
		{
			// do something with decryptedMessage.data
		})
